...
{
    // проверяет, передан ли явный урл.если он не пустой, то
    // мы сохраняем путь в переменную
    if ($url) {
        $redirect = $url;
        // если путь пустой, то если есть предыдущая ссылка мы присваевам ее переменной
        // если она отсутсвует то мы присваиваем переменной константу пути
        // типо значение по умолчанию 
    } else {
        $redirect = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : PATH;
    }

    // отправляет заголовок направления
    header("Location:{$redirect}");
    // нужно закончить программу, тк браузер может не успеть обработать редирект
    exit;
}
